...
- **CH-06** Give the customer `<script src="https://<app host>/chat-widget.js" data-widget-id="<id>" async></script>`. It answers as soon as it loads, so put it on a test page first.
...
- A widget cannot be deleted. Create one on purpose.
- The install snippet is not returned by any route. Build it from the app host and the widget id, and check it on a test page before it reaches a real page.
...
- `set_opening_hours` replaces the whole week (unconfirmed on 2026-09-10; check the live schema before you send it).
...
Putting the script tag on the real page. Publishing the A record for a custom
domain. Editing the portal layout. Tell the customer which of these are waiting
on them.
