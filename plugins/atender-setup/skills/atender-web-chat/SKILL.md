...
- **CH-06** No tool returns the install tag. Build it by hand from the widget id: `<script src="https://v3.atender.com/chat-widget.js" data-widget-id="<id>" async></script>`. It answers as soon as it loads, so put it on a test page first.
...
Widget fields, routing, after-hours behaviour and the custom-channel webhook
contract are in `references/widget-and-custom-channels.md`.

## Rules

- A channel answers only when its stack is enabled. Teams come first, then the stack, then the channel.
- **To take the AI off a widget: `update_chat_widget` with `aliMainAgentId: null` and `afterHoursMainAgentId: null`.** The widget stays up and people still get the chats — it is the AI that stops answering, not the channel.
- There is no public delete route for a widget, on purpose. A widget is deleted in the app. Create one deliberately.
- The install tag is not returned by any route. Build it by hand (CH-06) and check it on a test page before it reaches a real page.
- A push channel turns itself off after 10 failed deliveries in a row. A resend is a new delivery. `update_channels` `isActive: true` turns it on again.
...
- Never build a channel write body from a read. Send only the fields you change.
- Never delete or overwrite a channel unless the customer says yes to that channel by name.
...
record for a custom domain. Editing the portal layout. Tell the customer which of these are waiting
on them.
