...
- **HO-06** Opening hours, in order: `create_opening_hours_rule` with `timezone` as an IANA name (`Europe/Oslo`; the default is UTC) and `holidayCountry`, then `set_opening_hours_assignment` per team and channel. A pair with no assignment takes the default rule; no rule at all reads as always open.
