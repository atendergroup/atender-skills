...
- **SP-04** These two switches, not the stack's `knowledgeBaseEnabled`, gate what a specialist retrieves on a text channel. `kbEnabled: true` (default true) for the Knowledge Base. `handbookEnabled: true` on every specialist that must follow internal policy — its default is false **on purpose**, because the Handbook is internal and often empty, and a specialist that reads an empty Handbook gains nothing. Set it true where policy must be followed; never report the default as a fault.
